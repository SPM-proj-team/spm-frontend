<template>
    <div class="card">
        <div class="card-body">
            ...
            <div class="card-subtitle mb-1 text-muted d-flex justify-content-start align-items-start">
                <h6 class="me-2">
                    ...
                    {{ countSkillsMapped()}} / {{Skills.length}} Skill<template v-if="countSkillsMapped()>1">s</template> Mapped
                </h6>
            ...
            <li class="list-group-item d-flex justify-content-between align-items-start" v-for="Skill in Skills"
                ...
                <div class="ms-2 me-auto w-100">
                    <div class="d-flex justify-content-start">
                        ...
                        <div v-if="countCoursesPerSkill(Skill.Skill_ID)>0"
                            class="badge badge-pill bg-success">
                            {{ countCoursesPerSkill(Skill.Skill_ID) }} course<template v-if="countCoursesPerSkill(Skill.Skill_ID)>1">s</template>
                        </div>
                    </div>
        ...
        <button class="btn btn-warning rounded-0 rounded-bottom fw-bold" type="button">Create Learning Journey</button>
    </div>
</template>
